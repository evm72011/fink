...
{
    const european_call opt{.expiry = 1.0, .payoff = call_payoff{.strike = 100.0}};
    ...
    const double bs = bs_european_call(model.s0, 100.0, model.r, model.sigma, opt.expiry);
...
{
    const european_put opt{.expiry = 1.0, .payoff = put_payoff{.strike = 100.0}};
    const gbm_params model{.s0 = 100.0, .r = 0.05, .sigma = 0.0};
    const mc_config cfg{.paths = 10'000, .seed = 123};

    const auto mc = price_european_mc(opt, model, cfg);
    const double bs = bs_european_put(model.s0, 100.0, model.r, model.sigma, opt.expiry);
...
{
    const european_call opt{.expiry = 0.0, .payoff = call_payoff{.strike = 100.0}};
...
TEST(Pricers_European_MC, StandardErrorDecreasesWithMorePaths)
{
    ...
    const auto small = price_european_mc(opt, model, mc_config{.paths = 20'000, .seed = 123});
    const auto large = price_european_mc(opt, model, mc_config{.paths = 200'000, .seed = 123});

    // stderr should scale approximately like 1/sqrt(N)
    EXPECT_LT(large.stderr, small.stderr);

    const double expected_ratio = std::sqrt(20'000.0 / 200'000.0); // sqrt(N_small/N_large) = sqrt(0.1)
