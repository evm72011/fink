...
TEST(Pricers_BlackScholes, AtTheMoneyReferenceValue)
{
    ...
    // Call price ≈ 10.4506
    const double price = bs_call(100.0, 100.0, 0.05, 0.2, 1.0);
...
TEST(Pricers_BlackScholes, ZeroMaturityEqualsIntrinsicValue)
{
    ...
    EXPECT_DOUBLE_EQ(bs_call(120.0, 100.0, 0.05, 0.2, 0.0), 20.0);
    EXPECT_DOUBLE_EQ(bs_call(80.0, 100.0, 0.05, 0.2, 0.0), 0.0);
    EXPECT_DOUBLE_EQ(bs_call(100.0, 100.0, 0.05, 0.2, 0.0), 0.0);
}
...
TEST(Pricers_BlackScholes, ZeroVolatilityBecomesDiscountedForwardIntrinsic)
{
    ...
    const double price = bs_call(s0, k, r, /*sigma=*/0.0, T);
...
TEST(Pricers_BlackScholes, DeepInTheMoneyApproachesForwardIntrinsic)
{
    ...
    const double price = bs_call(s0, k, r, sigma, T);
...
TEST(Pricers_BlackScholes, MonotonicInSpot)
{
    ...
    const double c1 = bs_call(90.0, k, r, sigma, T);
    const double c2 = bs_call(100.0, k, r, sigma, T);
    const double c3 = bs_call(110.0, k, r, sigma, T);
...
TEST(Pricers_BlackScholes, MonotonicInVolatility)
{
    ...
    const double low = bs_call(s0, k, r, 0.10, T);
    const double mid = bs_call(s0, k, r, 0.20, T);
    const double high = bs_call(s0, k, r, 0.40, T);
...
TEST(Pricers_BlackScholes, MonotonicInMaturity)
{
    ...
    const double c_short = bs_call(s0, k, r, sigma, 0.25);
    const double c_mid = bs_call(s0, k, r, sigma, 1.0);
    const double c_long = bs_call(s0, k, r, sigma, 2.0);
...
TEST(Pricers_BlackScholes, NonNegativeAndUpperBoundedBySpot)
{
    ...
    const double c = bs_call(s0, k, r, sigma, T);
    EXPECT_GE(c, 0.0);
    EXPECT_LE(c, s0);
}
